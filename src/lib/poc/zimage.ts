...
async function createTask(prompt: string): Promise<string> {
  ...
  if (data.code !== 200 || !data.data?.taskId) {
    throw new Error(`KIE createTask failed: ${data.msg || "unknown"}`);
  }
...
async function pollTask(taskId: string): Promise<string> {
  for (let i = 0; i < POLL_MAX_TRIES; i++) {
    try {
      ...
      if (state === "failed") {
        throw new Error(`KIE failed: ${data.data?.failMsg || "unknown"}`);
      }
    } catch (e) {
      // 暫時性錯誤就重試，但若是 fail 訊息就拋出
      if (e instanceof Error && e.message.startsWith("KIE failed")) throw e;
    }
    await sleep(POLL_INTERVAL_MS);
  }
  throw new Error("KIE polling timeout (60s)");
}
...
export async function generateMany(
  ...
  if (urls.length === 0) {
    throw new Error("All KIE tasks failed");
  }
  return urls;
}
